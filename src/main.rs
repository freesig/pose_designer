...
struct Receivers {
    ...
    debug_info_rx: mpsc::Receiver<DebugMsg>,
}
...
fn model(app: &App) -> Model {
    ...
    let (debug_info_tx, debug_info_rx) = mpsc::channel();
    ...
    let mut detection_test = match design_mode {
        DesignMode::Test(ref pose_name, threshold) => {
            let settings = npe::Settings {
                joint_cutoff: 0.01,
            ...
            let poses: npe::PoseData = poses::read_poses()
                .into_iter()
                .map(|pr| {
                    let data = pr.data.into_iter()
                        .map(|j2d| (j2d_to_j(j2d.0), j2d.1.into()))
                        .collect();
                    (npe::Pose::from_name(&pr.name).expect("Failed to match pose name"), data)
                })
                .collect();
            let p_name = npe::Pose::from_name(pose_name).expect("Failed to match pose name");
            poses.get(&p_name)
                .map(|pose| {
                    let pose = pose.clone();
                    npe::Tester::test_pose(settings, npe::Pose::from_name(pose_name).expect("Failed to match pose name"), pose)
                })
        },
        _ => None,
    };
        ...
        */
        ...
        nui.skeleton_data(move |data| {
            ...
            let skeletons = data.skeletons()
                .iter()
                .map(|skeleton| {
                    if let Some(ref mut tester) = detection_test {
                        let pose = tester.test(&skeleton);
                        let pose_verts = tester.detector.lasts.pose.replace(None);
                        let skeleton_verts = tester.detector.lasts.skeleton.replace(None);
                        let rotation_debug = tester.detector.lasts.rotations.replace(None);
                        let max_dist_debug = tester.detector.lasts.max_dist.replace(None);
                        if let (Some(rotation), Some(max_dist)) = (rotation_debug, max_dist_debug) {
                            debug_info_tx.send(DebugMsg{ rotation, max_dist }).ok();
                        }
                        if let Ok(msg) = detection_rx.try_recv() {
                            match msg {
                                DetectionMsg::RotationCutOff(r) => tester.detector.settings.rotation_cutoff = r,
                                DetectionMsg::JointCutOff(j) => tester.detector.settings.joint_cutoff = j,
                            }
                        }
                        if let Some(pose_verts) = pose_verts {
                            debug_poses = pose_verts.iter().map(|v| vec2(v.x, v.y)).collect::<Vec<_>>();
                        }

                        if let Some(skeleton_verts) = skeleton_verts {
                            debug_skeletons = skeleton_verts.iter().map(|v| vec2(v.x, v.y)).collect();
                        }
                        println!("Pose: {:?}", pose);
                    }
                    if let Some(ref mut detector) = live_detection {
                        if let Ok(msg) = detection_rx.try_recv() {
                            match msg {
                                DetectionMsg::RotationCutOff(r) => detector.settings.rotation_cutoff = r,
                                DetectionMsg::JointCutOff(j) => detector.settings.joint_cutoff = j,
                            }
                        }
    ...
    let rx = Receivers { skeletons, depth, color, nui: nui_tx, skeleton_debug_rx, detection: detection_tx, debug_info_rx };
    ...
    let ids = Ids {
        ...
        rotation_cutoff: ui.generate_widget_id(),
        max_dist: ui.generate_widget_id(),
        rotations: ui.generate_widget_id(),
...
fn update(app: &App, model: &mut Model, _update: Update) {
    match model.rx.debug_info_rx.try_iter().last() {
        Some(DebugMsg{ max_dist, rotation }) => {
            model.display.max_dist = max_dist;
            model.display.rotation = rotation;
        }
        _ => (),
...
fn view(app: &App, model: &Model, frame: Frame) -> Frame {
    ...
    if let Some(ref debug_poses) = model.current.debug_poses {
        for debug_pose in debug_poses.iter() {
            let w = 16.0 / img_w as f32;
            let h = 16.0 / img_h as f32;
            let shift_x = debug_pose.x - 1.0 + 0.5;
            let shift_y = debug_pose.y * 2.0 - 1.0 + 0.5;
            let vs = Rect::from_w_h(w, h)
    ...
    if let Some(ref debug_skeletons) = model.current.debug_skeletons {
        for debug_skeleton in debug_skeletons.iter() {
            let w = 16.0 / img_w as f32;
            let h = 16.0 / img_h as f32;
            let shift_x = debug_skeleton.x - 1.0 + 0.5;
            let shift_y = debug_skeleton.y * 2.0 - 1.0 + 0.5;
            let vs = Rect::from_w_h(w, h)
                .shift([shift_x, shift_y].into())
                .triangles_iter()
                .flat_map(|tri| tri.vertices())
